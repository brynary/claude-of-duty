...
const TARGETS = {
  ...
  downtimeMean: { range: [12, 28], unit: 's', label: 'Downtime between engagements',
    why: 'FEEL_TARGET §6.1 [measured]: round-based inter-round break is 10-15s (BO3 `zombie_between_round_time`), NOT the 25-45s TDM engagement cadence — the document is explicit that a 30s break in a wave mode reads as a bug. Our measured gap is that break plus the approach time staging buys (§6.2, 5-11s), so 10-15 + 5-11 = 15-26s, bounded here at 12-28.' },
  downtimeMax: { range: [0, 45], unit: 's', label: 'Longest quiet stretch',
    why: 'A long gap is a dead patch where the player walks with nothing to do. Extra quiet in a wave mode should come from inside the encounter — approach, disengagement, flanking — not from a longer scheduled break.' },
